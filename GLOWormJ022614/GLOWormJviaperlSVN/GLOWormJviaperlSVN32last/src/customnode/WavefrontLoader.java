...
public class WavefrontLoader {
	...
	private void readVertex(boolean flipXcoords) {
		...
		////		FOR JSH
		if (objfile.contains("JSH_L4") && flipXcoords) {
			float x = 20803.52f;
			float y = 12784.513f;
			float z = -4789.9414f;
			float w = 50000;
			vertices.add(new Point3f(
					w*((Float.parseFloat(sp[1]) - x))/(x-14701),
			...
			//		w*((Float.parseFloat(sp[1]) - x))/(-x+4736)));

			////		FOR MEI ADULT rot corr xz
		} else if (objfile.contains("MeiAdult") && flipXcoords) {
			float x = 10800.0f;
			float y = 10816.0f;
			float z =  -8844.971f;
			float w = 50000;
			...
			float w = 50000;
			vertices.add(new Point3f(
					w*((Float.parseFloat(sp[1]) - x))/(-x+8829),
					w*(Float.parseFloat(sp[2]) - y)/(x-8829),
					w*(Float.parseFloat(sp[3]) -z)/(x-8829)));

			////		FOR L2_2
		} else if (objfile.contains("L2_2") && flipXcoords) {
			float x = 8002f;
			float y = 5648f;
			float z = -4090f;
			float w = 50000;
			vertices.add(new Point3f(
					w*((Float.parseFloat(sp[1]) - x))/(x-2804),
					w*(Float.parseFloat(sp[2]) - y)/(x-2804),
					w*(Float.parseFloat(sp[3]) -z)/(x-2804)));

			////	FOR L1_3
		} else if (objfile.contains("L1_3") && flipXcoords) {
			float x = 8316f;
			float y = 6042f;
			float z = -2470f;
			float w = 50000;
			vertices.add(new Point3f(
					-w*((Float.parseFloat(sp[1]) - x))/(x-4412),
			...
			//	-w*((Float.parseFloat(sp[1]) - x))/(x-8788)));

			////	FOR L1_2 fixing rot around Yaxis
		} else if (objfile.contains("L1_2") && flipXcoords) {
			float x = 12564f;
			float y = 7782f;
			float z =  -3865f;
			float w = 50000;
			vertices.add(new Point3f(
					-(float)(Math.cos(-0.495)*(w*(Float.parseFloat(sp[3]) -z)/(x-8788)) - Math.sin(-0.495)*(w*((Float.parseFloat(sp[1]) - x))/(x-8788))),
					w*(Float.parseFloat(sp[2]) - y)/(x-8788),
					-(float)(Math.sin(-0.495)*(w*(Float.parseFloat(sp[3]) -z)/(x-8788)) + Math.cos(-0.495)*(w*((Float.parseFloat(sp[1]) - x))/(x-8788)))));
			...
			//	w*((Float.parseFloat(sp[3]) - z))/(x-4464)));

			////		FOR L1_4 with yz rotation around x axis
		} else if (objfile.contains("L1_4") && flipXcoords) {
			float x = 8402f;
			float y = 12372f;
			float z =  -2725f;
			float w = 50000;
			vertices.add(new Point3f(
					w*(Float.parseFloat(sp[1]) - x)/(x-4464),
					(float)(Math.cos(-0.558)*(w*(Float.parseFloat(sp[2]) - y)/(x-4464)) - Math.sin(-0.558)*(w*((Float.parseFloat(sp[3]) - z))/(x-4464))),
					(float)(Math.sin(-0.558)*(w*(Float.parseFloat(sp[2]) - y)/(x-4464)) + Math.cos(-0.558)*(w*((Float.parseFloat(sp[3]) - z))/(x-4464)))));

			////	FOR L1_5
		} else if (objfile.contains("L1_5") && flipXcoords) {
			float x = 7990f;
			float y = 6999f;
			float z =  -3317f;
			float w = 50000;
			vertices.add(new Point3f(
					...
					w*((Float.parseFloat(sp[3]) - z))/(x-3973)));


			//	FOR N2Uadult
		} else if (objfile.contains("N2U_Adult") && flipXcoords) {
			float x = 19953f;
			float y = 15255f;
			float z =  -6105f;
			float w = 50000;
			vertices.add(new Point3f(
					...
					w*((Float.parseFloat(sp[3]) - z))/(x-9936)));
