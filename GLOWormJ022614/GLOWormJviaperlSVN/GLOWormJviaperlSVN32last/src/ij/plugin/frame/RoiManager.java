...
//		addPopupItem("Plot MK c-phate icospheres to coords");
		...
		addPopupItem("fuseAdjacentInSlice");
...
//						sortmode++;
			...
			} else if (command.equals("fuseAdjacentInSlice")) {
				this.fuseAdjacentRois(shiftKeyDown);
...
//					continue;
	...
	private void sketchVolumeViewer(Object source) {
	    ...
	    // TEMPORARY: Use a single-element array (named distinctly) to hold the mutable value 
	    // needed inside the final anonymous Runnable for inter-thread communication.
	    final double[] dialogScaleFactor = new double[]{1.0d};
	    final boolean[] wasCanceled = new boolean[]{false};
	    
	    // CRITICAL FIX: Forces the blocking GenericDialog to run on the AWT Event Dispatch Thread (EDT).
	    try {
	        SwingUtilities.invokeAndWait(new Runnable() {
	            public void run() {
	                ...
	                if (svvDialog.wasCanceled()) {
	                    ...
	                    // Value is stored in the array element
	                    dialogScaleFactor[0] = svvDialog.getNextNumber(); 
	                }
	            }
	        });
	    } catch (Exception e) {
	    ...
	    if (wasCanceled[0]) return;

	    // --- ORIGINAL VARIABLES RESTORED BELOW ---

	    // The value is unwrapped from the array and assigned back to the primitive variable 'sf'
	    double zPadFactor = 3;
	    double scaleFactor = dialogScaleFactor[0]; // <<< NOW 'sf' IS A PRIMITIVE DOUBLE, as expected.
	    boolean flip90forProj = false;
	    
	    IJ.setForegroundColor(255, 255, 255);		IJ.setBackgroundColor(0, 0, 0);
...
//		roiColorString = roiColorString.substring(3 /*roiColorString.length()-6*/);
		...
		// *** MEMORY FIX: Ensure 'MQTVS_VolumeViewer vv' is NOT declared before this loop. ***

		for (int n = 0; n < rootNames_rootFrames.size(); n++) {
		    
		    // 1. INSTANTIATE NEW UNIVERSE (Memory Leak Fix)
		    ...
		    // Check if files exist to skip processing
		    if (new File(outPathObj).canRead() 
		            && new File(outPathMtl).canRead() 
		            && new File(outPathGltf).canRead()) {
		        
		        // IMPORTANT: Even if skipping, we must kill the unused universe we just made!
		        if (vv.getUniv() != null) {
		            vv.getUniv().cleanup();
		        }
		        vv = null;
...
//							sketchImp.setPosition(1, nextSlice-zp, nextFrame);  ***
		    ...
		    // 2. HANG PREVENTION: Safety Timeout for Draw/Fill
		    ...
		    while (!filled) {
		        IJ.wait(100);
		        // Break out if we've waited 60 seconds
		        if (System.currentTimeMillis() - fillStart > 60000) {
		            IJ.log("Warning: ROI fill operation timed out for " + rootName);
		            break;
		        }
		    }
...
//					sketchImp.show();
		    ...
		    // Run the viewer (which now includes the deadlock fix from MQTVS_VolumeViewer.java)
		    ...
		    // 3. MEMORY FIX: Destroy the Universe
		    if (vv.getUniv() != null) {
		        vv.getUniv().cleanup();
		    }
		    vv.dispose();
		    ...
		    // THE NATIVE FLUSH PROTOCOL
		    System.runFinalization(); // 1. Tell the Finalizer thread to run NOW.
		    System.gc();              // 2. Mark new garbage.
		    System.runFinalization(); // 3. Run it again for anything freed by the GC.
		    
		    // 4. CRITICAL: Yield the CPU so the Native thread has time to actually free the RAM.
		    try { 
		        Thread.sleep(200); // 200ms pause is usually enough to drain the queue
...
//			}
		...
		LinkedHashMap<String, Double> rNRF_NeuronsVolumeLHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_NeuronsSA_LHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_PatchesSA_LHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_NeuronsTotalPatchesSA_LHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_PatchesRecipAvgSA_LHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_RankedNeuronVolumesOrderWholeBrainLHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_RankedNeuronSAsOrderWholeBrainLHM = new LinkedHashMap<String, Double>();                  
		LinkedHashMap<String, Double> rNRF_RankedPatchesSAsOrderWholeBrainLHM = new LinkedHashMap<String, Double>();                  

		LinkedHashMap<String, ArrayList<String>> rNRF_RankedCellVolumesPerBrainLHM = new LinkedHashMap<String, ArrayList<String>>();                  
		LinkedHashMap<String, ArrayList<String>> rNRF_RankedCellSAsPerBrainLHM = new LinkedHashMap<String, ArrayList<String>>();                  
		LinkedHashMap<String, ArrayList<String>> rNRF_RankedPatchSAsPerCellLHM = new LinkedHashMap<String, ArrayList<String>>();                  
		LinkedHashMap<String, ArrayList<String>> rNRF_RankedPatchSAsPerBrainLHM = new LinkedHashMap<String, ArrayList<String>>();                  
...
//			if (!rootName.contains("by")) {
			if (!rootNameFrame.matches("([a-z,A-Z,0-9,-]+)by([a-z,A-Z,0-9,-]+)(_\\d+)")) {
				...
				if (!rootNameFrame.contains("by")) {
					...
					rNRF_NeuronsTotalPatchesSA_LHM.put(rootNameFrame.replaceAll("(.*)(by.*)(_.*)", "$1$3"), 
							totalrNRF__SA + (rNRF_NeuronsTotalPatchesSA_LHM.get(rootNameFrame.replaceAll("(.*)(by.*)(_.*)", "$1$3"))!=null?
									rNRF_NeuronsTotalPatchesSA_LHM.get(rootNameFrame.replaceAll("(.*)(by.*)(_.*)", "$1$3")):
										0));
				}
				rNRF_PatchesSA_LHM.put(rootNameFrame, totalrNRF__SA);			
			}
		...
		for (int n = 0; n < rootNames_rootFrames.size(); n++) {
			...
			if (!rootName.contains("by") && rNRF_NeuronsTotalPatchesSA_LHM.get(rootNameFrame)!=null) {
				double rnfSA = rNRF_NeuronsSA_LHM.get(rootNameFrame);
				double rnfTPSA = rNRF_NeuronsTotalPatchesSA_LHM.get(rootNameFrame);
				IJ.log(""+rootName+", SA="+String.format("%.2f",rnfSA)+", TPSA="+String.format("%.2f",rnfTPSA));
				sumSAs = sumSAs + rnfSA;
				sumTPSAs = sumTPSAs + rnfTPSA;
			}
		}
		IJ.log("Totals: cellSAs = "+sumSAs+" cellPatchworkSAs = "+sumTPSAs);
		...
		for (String rNRF_A : rootNames_rootFrames) {
			...
			if (rNRF_PatchesSA_LHM.get(rNRF_A)!=null && rNRF_PatchesSA_LHM.get(rNRF_B)!=null) {
				
			} else {
				continue; //for rNRF_A
			}
			Double rNRF_RecipAvgSA = (rNRF_PatchesSA_LHM.get(rNRF_A) 
											+ rNRF_PatchesSA_LHM.get(rNRF_B))
											/ 2;
			if (!(rNRF_PatchesRecipAvgSA_LHM.containsKey(rNRF_A.replace("by", "and"))
				 ||rNRF_PatchesRecipAvgSA_LHM.containsKey((rNRF_B.replace("by", "and"))))) {
					rNRF_PatchesRecipAvgSA_LHM.put(rNRF_A.replace("by", "and"), rNRF_RecipAvgSA);
					rNRF_PatchesRecipAvgSA_LHM.put(rNRF_B.replace("by", "and"), rNRF_RecipAvgSA);	
					IJ.log(rNRF_A+","+rNRF_PatchesSA_LHM.get(rNRF_A));
					IJ.log((rNRF_A.replace("by", "and")+","+rNRF_PatchesRecipAvgSA_LHM.get(rNRF_A.replace("by", "and"))));
					IJ.log((rNRF_B.replace("by", "and")+","+rNRF_PatchesRecipAvgSA_LHM.get(rNRF_B.replace("by", "and"))));
					IJ.log(rNRF_B+","+rNRF_PatchesSA_LHM.get(rNRF_B));
					IJ.log("-");
			}
		}

		Double sumAllCellVolumes = 0d;
		Double sumAllCellSAs = 0d;
		Double sumAllRecipAvgPatchSAs = 0d;
		Double sumAllPatchSAs = 0d;
		
		ArrayList<String> sortedrNRF_neuronsbyVol = 
				rNRF_NeuronsVolumeLHM
			    .entrySet()
			    .stream()
		...
		ArrayList<String> sortedrNRF_neuronsbySA = 
				rNRF_NeuronsSA_LHM
			    .entrySet()
			    .stream()
			    .sorted(Map.Entry.comparingByValue())
		...
		ArrayList<String> sortedrNRF_sbyRecipAvgSA = 
				rNRF_PatchesRecipAvgSA_LHM
				.entrySet()
			    .stream()
			    .sorted(Map.Entry.comparingByValue())
			    .map(Map.Entry::getKey)
		...
		ArrayList<String> sortedrNRF_sbySA = 
				rNRF_PatchesSA_LHM
				.entrySet()
			    .stream()
			    .sorted(Map.Entry.comparingByValue())
...
//
//		ArrayList<String> sortedrNRF_sbyVolEachCell = 
//				rNRF_RankedVolumesOrderWholeBrainLHM
//			    .entrySet()
//			    .stream()
//			    .sorted(Map.Entry.comparingByKey())
//			    .sorted(Map.Entry.comparingByValue())
...
//		for (int vToBy=sortedrNRF_sbyVolEachCell.size()-1; vToBy>=0; vToBy--) {
//			rNRF_RankedVolumesOrderToCellLHM.put(sortedrNRF_sbyVolEachCell.get(vToBy).split("(by|_)")[0],rNRF_VolumeLHM.get(sortedrNRF_sbyVolEachCell.get(vToBy)));
//			IJ.log(sortedrNRF_sbyVolEachCell.get(vToBy).split("(by|_)")[0]+","+sortedrNRF_sbyVolEachCell.get(vToBy)+","+rNRF_VolumeLHM.get(sortedrNRF_sbyVolEachCell.get(vToBy)));
////			if(sortedrNRF_sbyVolEachCell.get(vToBy).split("(by|_)").length>1) {
////				rNRF_RankedVolumesOrderByCellLHM.put(sortedrNRF_sbyVolEachCell.get(vToBy).split("(by|_)")[1],rNRF_VolumeLHM.get(sortedrNRF_sbyVolEachCell.get(vToBy)));
////				IJ.log(sortedrNRF_sbyVolEachCell.get(vToBy).split("(by|_)")[1]+","+sortedrNRF_sbyVolEachCell.get(vToBy)+","+rNRF_VolumeLHM.get(sortedrNRF_sbyVolEachCell.get(vToBy)));
////			}
//		}
//	
//		ArrayList<String> sortedrNRF_sbySAEachCell = 
//				rNRF_RankedSAsOrderWholeBrainLHM
//			    .entrySet()
//			    .stream()
//			    .sorted( Map.Entry.comparingByKey())
//			    .sorted(Map.Entry.comparingByValue())
//			    .map(Map.Entry::getKey)
//			    .collect(Collectors.toCollection(ArrayList::new));
//		IJ.log("\nRanked By SA over each contacted cell");
//		for (int saToBy = sortedrNRF_sbySAEachCell.size()-1; saToBy>=0; saToBy--) {
//			rNRF_RankedSAsOrderToCellLHM.put(sortedrNRF_sbySAEachCell.get(saToBy).split("(by|and|_)")[0],rNRF_SA_LHM.get(sortedrNRF_sbySAEachCell.get(saToBy)));
//			IJ.log(sortedrNRF_sbySAEachCell.get(saToBy).split("(by|and|_)")[0]+","+sortedrNRF_sbySAEachCell.get(saToBy)+","+rNRF_SA_LHM.get(sortedrNRF_sbySAEachCell.get(saToBy)));
////			if(sortedrNRF_sbySAEachCell.get(saToBy).split("(by|_)").length>1) {
////				rNRF_RankedSAsOrderByCellLHM.put(sortedrNRF_sbySAEachCell.get(saToBy).split("(by|_)")[1],rNRF_SA_LHM.get(sortedrNRF_sbySAEachCell.get(saToBy)));
////				IJ.log(sortedrNRF_sbySAEachCell.get(saToBy).split("(by|_)")[1]+","+sortedrNRF_sbySAEachCell.get(saToBy)+","+rNRF_SA_LHM.get(sortedrNRF_sbySAEachCell.get(saToBy)));
////			}
//		}
...
//			}
	...
	public void setUpRoisByNameAndNumbers(Roi roi) {
		...
		//						sliceRois.add(roi);
		if (roi.getName().contains("~") || roi.getName().contains("by") || roi.getName().matches("\"[A-Z]+in.* .*")){
			sliceRois.add(0,roi);
		}else {
			sliceRois.add(roi);
		}
	...
	public boolean delete(boolean replacing) {
		...
		Roi listRoi = null;
		if (indexes.length == 1) {
			listRoi = rois.get(listModel.get(indexes[0]));
			listRoi.setImage(imp);
		}
		Roi impRoi = imp.getRoi();
		if (impRoi != null &&  !impRoi.equals(listRoi)) {
...
//				c =0;
			...
			IJ.showStatus(""+i+" of "+indexes.length+" tags renamed...");
...
//		IJ.showStatus("Finished. All " + indexes.length+" tags renamed.");
...
//				//????why is this even necessary if the roimanager already made all names unique????
//				int hitCount = 0;
//				while (namesAlreadySaved.contains(labelNew)) {
//					hitCount++;
//					labelNew = labelNew.replaceAll("(.*)(-[0-9]+)?(.roi)", "$1-" + hitCount + "$3");
//				}
//				//???Comment the above out and test???
...
//			}
		...
		textFilterField.setEnabled(false);
		textFindingField.setEnabled(false);
		for (int i = 0; i < n; i++) {
			String label = (String) listModel.getElementAt(indexes[i]);
			Roi roi = (Roi) rois.get(label);
		...
		textFilterField.setEnabled(true);
		textFindingField.setEnabled(true);
...
//		rename(newNames,  indexes, true);
	...
	public Roi[] getFullRoisAsArray() {
		return getFullRoisAsArray(false);
	}
	...
	public void select(ImagePlus imp, int index) {
		...
		if (list.getSelectedIndices().length <= 1) {
			...
			imp.getRoi().setState(Roi.NORMAL);
...
//		Roi[] roiSetIn = this.getFullRoisAsArray(alphaSortList);
	...
	public Roi[] getSliceSpecificRoiArray(int z, int t, boolean getSpanners, boolean alphaSortList) {
		Roi[] roiSetIn = this.getFullRoisAsArray(alphaSortList);
		if (roiSetIn == null)
			return null;
		ArrayList<Roi> matchedRois = new ArrayList<Roi>();
	...
	public int[] getSliceSpecificIndexes(int z, int t, boolean getSpanners, boolean alphaSortList) {
		Roi[] roiSetIn = this.getFullRoisAsArray(alphaSortList);
		if (roiSetIn == null)
			return null;
		ArrayList<Integer> matchedIndexes = new ArrayList<Integer>();
...
// THIS LOOP CASTS THE SEARCH ALONG THE ENTIRE LENGTH OF EACH SELECTED CELL.  
			ArrayList<Roi> soNamedRois = this.getROIsByName().get("\"" + roi.getName().split("\"")[1] + "\"");
			
			for (Roi queryRoi : soNamedRois) {
				String roiNameRoot = roi.getName().split("\"")[1];
				if (!queryRoi.getName().split("\"")[1].equalsIgnoreCase(roiNameRoot)) {
					continue;
				}
//	WITH justSelectedSlices, NEIGHBOR SEARCH ONLY IN SELECTED SLICE, NOT ALL SLICES
				...
				for (Roi testRoi : sameSliceRois) {
					if (zPos != testRoi.getZPosition()
							...
							|| queryRoi.getName().split("\"")[1].trim().contains("by")) {
						continue;
					}
					if ((testRoi.getCPosition() != queryRoi.getCPosition()
							|| testRoi.getZPosition() != queryRoi.getZPosition()
							|| testRoi.getTPosition() != queryRoi.getTPosition())){
						continue;
					}
					...
					ShapeRoi testShapeRoi = new ShapeRoi(testRoi);
//					imp.setRoi(scaledShapeRoi);
//					IJ.wait(1);
					Roi[] shTestComponentRois = testShapeRoi.getRois();
...
//											IJ.wait(1);
								...
								//Capture the final point chain from the double run-around
...
//										IJ.wait(1);
									...
									for (int csp=0; csp<contactSegPts.size(); csp++) {
										...
										if (contactSegXs.size()==1)
											IJ.wait(1);
									}
									// Nice java8 stream().mapToInt() trick!!
									int[] csxs = contactSegXs.stream().mapToInt(i -> i).toArray();
									int[] csys = contactSegYs.stream().mapToInt(i -> i).toArray();
									PolygonRoi contactSegmentRoi = new PolygonRoi( csxs, csys, csxs.length, PolygonRoi.POLYLINE);
...
//						(double)(((p/w)-imp.getCalibration().yOrigin)*imp.getCalibration().pixelHeight));
	...
	public void translateMapColors() {
		...
		int countY = 0;
		int hitCount = 0;
		double qCutoff = IJ.getNumber("Max acceptable q value: ", 0.5);
...
//				int p = (int)((y-hStep*centeringFraction)/hStep)*gridN + (int)((x-wStep*centeringFraction)/wStep);
				...
				if (colorIntToCalXYlut.get(""+colorInts[countXY][0]+"_"+colorInts[countXY][1]+"_"+colorInts[countXY][2])!= null) {
					...
					boolean qAccepted = colorIntToCalXYlut.get(""+colorInts[countXY][0]+"_"+colorInts[countXY][1]+"_"+colorInts[countXY][2])[0] > -qCutoff;
					...
					if (qAccepted) {
						...
						IJ.log("OnChart: "+randiNeuronNames[countX]+"to"+randiNeuronNames[countY] 
								+" "+ hexColorCode 
								+" "+ (qAccepted?"√":"x") +"q="+ colorIntToCalXYlut.get(""+colorInts[countXY][0]+"_"+colorInts[countXY][1]+"_"+colorInts[countXY][2])[0]
