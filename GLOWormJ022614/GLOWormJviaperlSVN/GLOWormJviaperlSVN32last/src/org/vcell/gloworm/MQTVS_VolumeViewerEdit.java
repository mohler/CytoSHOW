...
public class MQTVS_VolumeViewerEdit  implements PlugIn, WindowListener {
	                ...
	                try {
	                    ...
	                    if (context.singleSave) {
	                        ...
	                        MeshExporter.saveAsWaveFront(newestContent.values(), 
	                                new File(finalOutDir + File.separator + fileName), 
	                                univ.getStartTime(), univ.getEndTime(), true);

	                        // === 2. REMOVAL UI ACTION (Dispatch to EDT) ===
                            // FIX 2: Skip select/unlock and go straight to remove to avoid IndexOutOfBoundsException
