		...
		try {
			...
			if (false /*mqtf.length == 1*/) {	
				...
				if (virtualHyperStack) {
					vstack = new MultiQTVirtualStack(mqtf, new ArrayList<String>(), new ArrayList<String>(), movieSlices, eightBit, imp, stretchToFitOverlay, viewOverlay, sideSideStereo, redCyanStereo, horizontal, grid);	
				} else {
...
//*************IS THERE ANY REASON I CAN'T BUILD THIS imp WITHIN THE CONSTRUCTOR OF MultiQTVirtualStack, RATHER THAN IN THIS LINE OF THIS CODE???				
				...
				vstack = new MultiQTVirtualStack(mqtf, new ArrayList<String>(), new ArrayList<String>(), movieSlices, eightBit, imp, stretchToFitOverlay, viewOverlay, sideSideStereo, redCyanStereo, horizontal, grid);		//!!!!! In MultiQTVirtualStack I need a way to have one stack built from the array of QTFiles.
